...
if globalVariables.pretrained:
    globalVariables.deepQNetwork1.load_state_dict(torch.load('C:/Users/Rickard/Documents/python/MachineLearning/PythonSnakeDeepQ/savedNetworks/network12times8.pt'))
...
def greedy(state):
    ...
    with torch.no_grad():
        ...
        action = np.argmax(qValues.detach().numpy())
...
def calculateQLearningTargets():

    with torch.no_grad():
        qlearningTargets = torch.zeros(len(globalVariables.replayMemory))

        for i, sample in enumerate(globalVariables.replayMemory):
            reward = sample[2]
            nextState = sample[3]
            isTerminalState = sample[4]

            if globalVariables.deepQNetwork1Frozen:
                nextFrozenQValues = globalVariables.deepQNetwork1(nextState)
            else:
                nextFrozenQValues = globalVariables.deepQNetwork2(nextState)

            nextMaxFrozenQValue = torch.max(nextFrozenQValues)
            ...
            if not isTerminalState:
                currentTarget = reward + discountFactor*nextMaxFrozenQValue
            else:
                currentTarget = reward

            qlearningTargets[i] = currentTarget
...
def train():
    ...
    states = torch.zeros((len(globalVariables.replayMemory), 4, globalVariables.downSampleWidth, globalVariables.downSampleHeight))
    actions = torch.zeros(len(globalVariables.replayMemory))
    for i, sample in enumerate(globalVariables.replayMemory):
        states[i,:,:,:] = globalVariables.replayMemory[i][0]
        actions[i] = globalVariables.replayMemory[i][1]
    

    # Calculate the Q-learning targets
    qLearningTargets = calculateQLearningTargets()

    # Create the training dataset
    dataset = torch.utils.data.TensorDataset(states, qLearningTargets, actions)

    # Convert to a dataset
    datasetLoader = torch.utils.data.DataLoader(dataset, batch_size=globalVariables.miniBatchSize,
                                         shuffle=True, num_workers=0)


    for epoch in range(globalVariables.numberOfEpochs):
        for i, data in enumerate(datasetLoader, 0):

            states, targets, actions = data
            ...
            qValues = torch.zeros(actions.size(0))
            for i in range(actions.size(0)):
                action = actions[i]
                qValues[i] = qValuesAll[i, action]

            loss = lossFunction(qValues, targets)
            loss.backward()
...
while 1:
    ...
    globalVariables.qBuffer.append(qValues.detach().numpy())
    ...
    if oldState is not None:
        globalVariables.replayMemory.append((oldState, action, reward, stackedFrames, isTerminalState))
